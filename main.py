...
def create_pdf(user_details, damage_status, drowsiness_status, bill_details, story):
    ...
    if 'Items' in bill_details:
        ...
        for item in bill_details['Items']:
            pdf.cell(
                0, 10, f"- {item['Part']}({item['Car Make']} {item['Car Model']} - {item['Year']}): Rs{item['Price']: .2f}", ln=True)
        pdf.ln(5)
...
def get_gpt4_response(prompt):
    try:
        openai.api_key = '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'
